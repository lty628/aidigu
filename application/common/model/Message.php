...
class Message extends Model
{
	...
	public static function getMessage($userid = '', $count = 50, $topicId = 0)
	{
		...
		if ($topicId) {
			return self::with('user')->where('uid',$userid)->where('topic_id', $topicId)->where('is_delete', 0)->order('msg_id','desc')->paginate($count, false, ['page' => request()->param('page/d', 1), 'path' => '[PAGE].html']);
