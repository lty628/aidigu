    ...
    public static function initMessageData($data, $listtagid = '')
    {
        ...
        if ($data['content_type'] == 'mp3') {
            ...
            if ($listtagid == 'PrivateLetter') {
                $data['content'] = '<img style="max-width: 100px;" class="massageImgCommon massageImg_jpg" onclick="showMessageImg(this)" src="/static/upload/images/jpg-1.svg" data-src="'. $data['content']. '">';
            } else {
                $data['content'] = '<img style="max-width: 150px;" class="massageImgCommon massageImg_jpg" onclick="showMessageImg(this)" src="' . $data['content'] . '">';
            }
